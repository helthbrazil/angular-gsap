...
export class EffectsComponent extends BaseAnimatedComponent implements OnInit, AfterViewInit, OnDestroy {
    @ViewChild('shaderCanvas', { static: false }) canvasRef!: ElementRef<HTMLCanvasElement>;
    @ViewChild('shaderBackground', { static: false }) shaderBgRef!: ElementRef<HTMLDivElement>;
    @ViewChild('badge', { static: false }) badgeRef!: ElementRef<HTMLDivElement>;
    @ViewChild('heroTitle', { static: false }) titleRef!: ElementRef<HTMLHeadingElement>;
    @ViewChild('heroDescription', { static: false }) descriptionRef!: ElementRef<HTMLParagraphElement>;
    @ViewChild('ctaButtonsContainer', { static: false }) ctaRef!: ElementRef<HTMLDivElement>;
    @ViewChild('microDetailsContainer', { static: false }) microRef!: ElementRef<HTMLUListElement>;

    // Hero props
    title = "Where algorithms become art.";
    description = "A minimal hero with a neural canvas — crisp, elegant, and quietly expressive. Built with Angular, Three.js, and a custom CPPN shader.";
    ...
    // Tab state
    activeTab: 'neural' | 'scroll' = 'neural';
    ...
    initializeAnimations(): void {
        if (this.activeTab === 'neural') {
            // Check if canvas is available (it might not be if *ngIf hasn't rendered yet)
            if (!this.canvasRef) {
                console.warn('Canvas not ready, retrying...');
                setTimeout(() => this.initializeAnimations(), 50);
                return;
            }
            this.initThreeJS();
    ...
    switchTab(tab: 'neural' | 'scroll'): void {
        this.activeTab = tab;
        ...
        if (tab === 'scroll') {
            ...
            // Reinitialize neural network effect
            setTimeout(() => {
                this.initializeAnimations();
            }, 100);
        }
    ...
    private initThreeJS(): void {
        ...
        this.renderer.setClearColor(0x000000, 1); // Black background as fallback
    ...
    private animate = (): void => {
        ...
        // Update shader time
        this.material.uniforms['iTime'].value += 0.016; // ~60fps
    ...
    private initGSAPAnimations(): void {
        // Animate shader background entrance
        if (this.shaderBgRef) {
    ...
    private animateContent(): void {
        ...
        // Badge
        if (this.badgeRef) {
            tl.to(this.badgeRef.nativeElement, {
                autoAlpha: 1,
                y: 0,
                duration: 0.5
            }, 0.0);
        ...
        // Title (split by lines would require SplitText plugin, using simple animation)
        if (this.titleRef) {
            gsap.set(this.titleRef.nativeElement, {
                filter: 'blur(16px)',
                y: 30,
                autoAlpha: 0,
                scale: 1.06
            ...
            tl.to(this.titleRef.nativeElement, {
                filter: 'blur(0px)',
                y: 0,
                autoAlpha: 1,
                scale: 1,
        ...
        if (this.descriptionRef) {
            tl.to(this.descriptionRef.nativeElement, {
                autoAlpha: 1,
                y: 0,
                duration: 0.5
            }, '-=0.55');
        ...
        // CTA Buttons
        if (this.ctaRef) {
            tl.to(this.ctaRef.nativeElement, {
                autoAlpha: 1,
                y: 0,
                duration: 0.5
            }, '-=0.35');
        ...
        // Micro Details
        if (this.microRef) {
            const microItems = this.microRef.nativeElement.querySelectorAll('.micro-item');
            tl.to(microItems, {
                autoAlpha: 1,
                y: 0,
                duration: 0.5,
